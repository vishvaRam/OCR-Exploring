...
from dots_ocr.utils.doc_utils import pymupdf_doc_to_image
...
def get_image_by_pymupdf_doc(image, target_dpi=200):
    # get image through pymupdf, to get target dpi image, mainly for higher image
    ...
    pdf_bytes = pymupdf.open(stream=data_bytes).convert_to_pdf()
    doc = pymupdf.open('pdf', pdf_bytes)
    page = doc[0]
    image_pymupdf = pymupdf_doc_to_image(page, target_dpi=target_dpi, origin_dpi=origin_dpi)

    return image_pymupdf
